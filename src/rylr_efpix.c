...
//=============================================================================================
LIBEFPIX_Identity node;
uint8_t stored_hashes[MAX_HASHES][LIBEFPIX_HASH_SIZE];
int hash_count = 0;
LIBEFPIX_Contact contacts[MAX_CONTACTS];
int contact_count = 0;
int ignore;
//=============================================================================================
bool hash_check_and_relay(uint8_t hash[LIBEFPIX_HASH_SIZE], uint8_t packet[LIBEFPIX_PACKET_SIZE]) {
    for (int i = 0; i < hash_count; i++) {if (memcmp(stored_hashes[i], hash, LIBEFPIX_HASH_SIZE) == 0) return false;}
    if (hash_count < MAX_HASHES) {memcpy(stored_hashes[hash_count], hash, LIBEFPIX_HASH_SIZE); hash_count=hash_count+1;}
    ignore=packet[0];
    return true;
}
bool get_contact_from_alias(uint8_t alias[LIBEFPIX_ALIAS_SIZE], LIBEFPIX_Contact* contact) {
    for (int i = 0; i < contact_count; i++) {if (memcmp(contacts[i].their_alias, alias, LIBEFPIX_ALIAS_SIZE) == 0) {memcpy(contact, &contacts[i], sizeof(LIBEFPIX_Contact)); return true;}}
    return false;
}
...
void add_contact(LIBEFPIX_Contact contacts[MAX_CONTACTS], int* contact_count, const char* their_alias_str, uint8_t kx_public_key[32], uint8_t sign_public_key[32], const char* my_alias_str) {
    if (*contact_count < 10) {
...
void alias_copy(uint8_t dest[LIBEFPIX_ALIAS_SIZE], const char* src) {
    memset(dest, 0, LIBEFPIX_ALIAS_SIZE);
    strncpy((char*)dest, src, LIBEFPIX_ALIAS_SIZE - 1);
    dest[LIBEFPIX_ALIAS_SIZE - 1] = '\0';
}